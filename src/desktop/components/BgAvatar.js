 ...
 * @returns string
 */
export default function BgAvatar(name) {
    ...
    if (firstVal === 0) {
        firstVal = (c * 2 - minVal) / (maxVal - minVal) * 255;
    }
    let secondVal = 255 - firstVal,
        thirdVal = (firstVal * 2) % 256;
    return `linear-gradient(
        0deg, 
        rgba(${firstVal * 0.6}, ${secondVal * 0.6}, ${thirdVal * 0.6}, 1) 0%, 
        rgba(${firstVal}, ${secondVal}, ${thirdVal}, 1) 100%
    );`
}